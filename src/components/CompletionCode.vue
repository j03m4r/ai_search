<script setup lang="ts">
    import { onMounted, ref } from 'vue';
    import { useParticipant } from '@/composables/useParticipant';

    const { getCompletionCode } = useParticipant()
    const code = ref("")

    onMounted(async () => {
        code.value = await getCompletionCode();
    });
</script>

<template>
    <div class="w-full h-full flex justify-center items-start">
        <div class="w-7xl flex flex-col items-center justify-center">
            <div class="sticky top-0 w-full flex justify-start items-center bg-[#faf9f6] py-6 z-50">
                <h2 class="text-3xl font-bold! sticky top-0 bg-[#faf9f6] text-gray-600 w-full">Completion code</h2>
            </div>

            <p class="text-gray-600 text-2xl w-full mb-3!">
                Thank you for your time and participation! Here is your Prolific completion code: {{ code }}
            </p>
        </div>
    </div>
</template>
